...
jQuery("#show-more-partners").click(function () {
    console.log("Click!");
    jQuery(".single-partner.show-more").toggleClass("show");
    jQuery(this).toggleClass("active");
    if (jQuery(this).text() == "Show more") {
        jQuery(this).text("Show less");
    } else {
...
jQuery("#newsletter-form").submit(function(e){
    ...
    var form = this;
    var email = jQuery(form).find('[type="email"]').val();
    var formResponse = jQuery(".form-response");

    formResponse.text('');

    if(email == ""){
        formResponse.text("Email cannot be empty!");
        return;
    }
    ...
    .done(function(data){
        if(data.success){
            $(form).html("<p>" + data.success_msg + "</p>");
        } else {
            $(formResponse).text(data.error_msg);
        }
    })
